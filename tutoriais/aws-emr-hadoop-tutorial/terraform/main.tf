...
resource "aws_emr_cluster" "wordcount" {
  ...
  step {
    ...
    hadoop_jar_step {
      ...
      args = [
        "bash", "-c",
        "set -e && aws s3 cp s3://${local.bucket_name}/jars/wordcount.jar /tmp/wordcount.jar && hadoop jar /tmp/wordcount.jar WordCountApplication hdfs:///input/ hdfs:///output/wordcount/"
      ]
    }
  }
  ...
  step {
    ...
    hadoop_jar_step {
      ...
      args = [
        "bash", "-c",
        "set -e && rm -rf /tmp/wc-output && mkdir -p /tmp/wc-output && hdfs dfs -copyToLocal hdfs:///output/wordcount/* /tmp/wc-output/ && aws s3 cp /tmp/wc-output/ s3://${local.bucket_name}/output/ --recursive && echo COPY_DONE"
      ]
    }
  }
